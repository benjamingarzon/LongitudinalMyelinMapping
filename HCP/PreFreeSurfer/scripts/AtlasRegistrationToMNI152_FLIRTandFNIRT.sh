...
mri_nu_correct.mni --i ${T1wRestoreBrain}.nii.gz --o $WD/${T1wRestoreBrainBasename}_nu.nii.gz
mri_nu_correct.mni --i ${T1wRestore}.nii.gz --o $WD/${T1wRestoreBasename}_nu.nii.gz

${FSLDIR}/bin/flirt -interp spline -dof 12 -in $WD/${T1wRestoreBrainBasename}_nu.nii.gz -ref ${ReferenceBrain} -omat ${WD}/xfms/acpc2MNILinear.mat -out ${WD}/xfms/${T1wRestoreBrainBasename}_to_MNILinear

${FSLDIR}/bin/fnirt --in=$WD/${T1wRestoreBasename}_nu.nii.gz --ref=${Reference2mm} --aff=${WD}/xfms/acpc2MNILinear.mat --refmask=${Reference2mmMask} --fout=${OutputTransform} --jout=${WD}/xfms/NonlinearRegJacobians.nii.gz --refout=${WD}/xfms/IntensityModulatedT1.nii.gz --iout=${WD}/xfms/2mmReg.nii.gz --logout=${WD}/xfms/NonlinearReg.txt --intout=${WD}/xfms/NonlinearIntensities.nii.gz --cout=${WD}/xfms/NonlinearReg.nii.gz --config=${FNIRTConfig}
